 ...
 * Schema engine.
 *
 * Outputs Article and FAQPage JSON-LD on single posts. Defers Article to an
 * active SEO plugin when running in auto mode.
 *
...
class QuotedEasy_AI_Readiness_Schema {
	...
	private function print_jsonld( $data, $tag ) {
		...
		// Hex-encode <, >, &, ', " so user content cannot break out of the script tag.
	...
	private function extract_faqs( $post ) {
		...
		// [faq_item question="..."]Answer[/faq_item]
		...
		// Heuristic: H2/H3 ending in "?" followed by paragraph text.
		if ( empty( $pairs ) ) {
